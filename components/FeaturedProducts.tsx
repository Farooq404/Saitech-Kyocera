      ...
      <div className="container-xl">
        ...
        <div ref={gridRef} className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {products.map((p) => (
            ...
            >
              {p.badge && (
                <span className="self-start mb-4 text-[11px] font-display font-bold uppercase tracking-wide bg-accent-orange text-white px-2.5 py-1 rounded-full">
                  {p.badge}
                </span>
              )}
              {p.image ? (
                <Link
                  ...
                  className="w-full h-48 mb-5 rounded-xl overflow-hidden bg-gray-50 flex items-center justify-center cursor-pointer group-hover:bg-brand-blueLight/30 transition-colors"
                >
                  <img
                    src={p.image}
                    alt={p.name}
              ...
              <Link
                href={`/products/${p.slug}`}
                className="mt-auto inline-flex items-center gap-1.5 font-display font-semibold text-sm text-brand-blue group-hover:gap-2.5 transition-all"
              >
